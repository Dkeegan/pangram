let pangram (inFile : string) (outFile : string) : unit =
  ...
  let clist = ['a';'b';'c';'d';'e';'f';'g';'h';'i';'j';'k';'l';'m';'n';'o';'p';'q';'r';'s';'t';'u';'v';'w';'x';'y';'z'] in 


  
  let rec checkString str cl = 
   match cl with 
   head::tail-> 
      if (String.contains str head)
      then checkString str tail
      else false

   | []->   true in

   let rec restOFStrings sl cl =
   match sl with 
   head::tail ->
      (file_write( checkString head cl );
      restOFStrings tail cl)
      
   |[]->  () in
   restOFStrings ls_str clist ;;
